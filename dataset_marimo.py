...
def _(np):
    def encode_fen(fen):
        ...
        return np.packbits(np.concatenate([piece_planes, side, castle, ep]).astype(np.uint8)).tobytes()
...
@app.cell
def _(duckdb, np, torch):
    text = duckdb.sql("SELECT * FROM chessdb.evaluations LIMIT 4").fetchall()
    text[0][1]
    bits = np.unpackbits(np.frombuffer(text[0][1], dtype=np.uint8)).astype(np.float32)[:837]
    tensor = torch.tensor(bits)
    return tensor, text
...
@app.cell
def _(IterableDataset, duckdb, np, text, torch):
...
@app.cell
def _(lightning, nn, torch):

    class ChessModel(lightning.LightningModule):
        def __init__(self, lr=0.01, batch_size=1024, layer_count=6, in_feat=808, out_feat=1):
            super().__init__()
            self.lr = lr
        ...
        def training_step(self, batch, batch_idx):
            ...
            self.log("train_loss", loss) # Log automatically
            return loss
...
@app.cell
def _(ChessDataset, ChessModel, DataLoader):
    model = ChessModel(in_feat=837)
    print(f"Model:\n{model.seg}")
    print(f"Parameters: {sum(p.numel() for p in model.parameters()):,}")

    dataset = ChessDataset("chess_dataset.db", "evaluations")
    print(f"Dataset rows: {dataset.count:,}")

    dataloader = DataLoader(dataset, batch_size=1024, num_workers=4)
    print(f"Batch size: {dataloader.batch_size}")
    print(f"Workers: {dataloader.num_workers}")
    print(f"Batches per epoch: {dataset.count // dataloader.batch_size:,}")

    #trainer = lightning.Trainer(max_epochs=1, accelerator="mps")
    #trainer.fit(model, dataloader)
    return dataloader, model


@app.cell
def _(dataloader, lightning, model):
    from lightning.pytorch.loggers import MLFlowLogger

    logger = MLFlowLogger(experiment_name="chess-eval", tracking_uri="file:./mlruns")
